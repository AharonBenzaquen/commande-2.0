...
export default function App() {
  ...
  const [filtreAdmin, setFiltreAdmin] = useState('laval');

  const seConnecter = () => {
    if (utilisateurs[login] && utilisateurs[login].password === mdp) {
      setUtilisateur({ email: login, role: utilisateurs[login].role });
    } else {
  ...
  const ajouterCommande = () => {
    ...
    if (utilisateur.role !== 'admin') {
      nouvelleCommande.magasin = utilisateur.role;
    }
  ...
  const commandesVisibles = commandes.filter((c) => {
    ...
    if (utilisateur.role === 'admin') return c.magasin === filtreAdmin;
    if (utilisateur.role === 'labo') return true;
    return c.magasin === utilisateur.role;
  ...
  return (
    <div style={{ padding: 20 }}>
      <h2>Bienvenue ({utilisateur.role})</h2>
      <button onClick={() => setUtilisateur(null)}>Déconnexion</button>

      {utilisateur.role === 'admin' && (
        <div>
          <label>Filtrer par magasin : </label>
          <select value={filtreAdmin} onChange={(e) => setFiltreAdmin(e.target.value)}>
      ...
      <table border="1" cellPadding="6" style={{ marginTop: 10, width: '100%' }}>
        <thead>
          <tr>
            ...
            <th>Délai</th>
            <th>Commentaire</th>
            {utilisateur.role === 'labo' && <th>Magasin</th>}
            <th>Actions</th>
          </tr>
        </thead>
        <tbody>
          {commandesVisibles.map((c, i) => {
            ...
            return (
              <tr key={i} style={style}>
                ...
                {utilisateur.role === 'labo' && <td>{c.magasin}</td>}
                <td>
