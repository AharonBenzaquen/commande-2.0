...
export default function App() {
  ...
  const ajouterCommande = () => {
    const nouvelle = { ...commande, origine: role };
    if (editionIndex !== null) {
      const modifiees = [...commandes];
      modifiees[editionIndex] = nouvelle;
      setCommandes(modifiees);
      setEditionIndex(null);
    } else {
      setCommandes([...commandes, nouvelle]);
    }
    setCommande({ numero: '', client: '', date: '', statut: 'En attente', commentaire: '', origine: role });
  };
  ...
  const changerStatut = (index, statut) => {
    const copie = [...commandes];
    copie[index].statut = statut;
    setCommandes(copie);
  };
  ...
  const commandesFiltrees = commandes.filter((c) => {
    const visible = role === 'admin' || role === 'labo' || c.origine === role;
